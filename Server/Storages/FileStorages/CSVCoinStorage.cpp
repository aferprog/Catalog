#include"csv.hpp"
#include"CSVCoinStorage.h"
#include<iomanip>
#pragma message("Compiling CSVStorage.cpp")
static void fillCoin(CoinPtr coin, csv::CSVRow& row);
std::vector<CoinPtr> CSVCoinStorage::getAllCoins()const
{
...
size_t CSVCoinStorage::writeCoins(std::vector<CoinPtr>& coins)
{
...
CSVCoinStorage::CSVCoinStorage(std::string path):FileCoinStorage(path)
{
    format.delimiter(',');
    format.header_row(0);
}
